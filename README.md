...
そのため、LLM出力を評価する際は、案件の目的やリスクに応じて、評価観点・評価基準・合格基準を設計しなければなりません。しかし、その作り方や進め方が共通化されていないと、案件ごとに評価方法をゼロから考える必要があり、評価設計や判断が、評価する人に依存します。
...
そこで、この状態を解消するために、評価区分を階層化して整理し、完了条件から評価観点・評価基準・合格基準へ展開する流れを共通化しました。
...
![評価区分と完了条件、具体化する順番とフォーマットは共通化し、対象判定や評価観点、評価基準、合格基準の具体的な内容は案件ごとに決めることを示した図](assets/02-standardization-boundary.svg)

図の上段のように、評価区分と完了条件、具体化する順番、成果物のフォーマットを共通化しました。一方、評価対象とする完了条件や、評価観点・評価基準・合格基準の具体的な内容は、図の下段のように案件ごとに決める構造としました。
...
何も標準化されていない場合、評価する人は毎回次のことを考えなければなりません。
...
判断することが多いほど、評価設計の負担が増え、評価する人によって評価方法も変わります。

一方で、完了条件の対象判断、評価観点、評価基準、合格基準までをすべて固定してしまうと、案件固有の目的やリスクに対応できなくなります。
...
共通化できる部分はあらかじめ用意し、案件固有の判断が必要な部分だけを評価する人が考える構造としました。

## 最後に

冒頭でも触れたとおり、LLMの出力は、同じ入力でも毎回同じになるとは限らず、単純な○×だけでは評価できないこともあります。そのため、不確実性を伴うLLM出力を安定して評価できるよう、評価の進め方には共通の土台が必要です。

今回の取り組みを通して、こうしたLLM出力の評価プロセスを標準化する上で大切だと感じたのは、使う側に不要な判断をさせないことと、案件ごとの目的や特性に合わせられる柔軟性を残すことの両立です。

フォーマットや作業の順番を共通化しておけば、使う側は本来考えるべき案件固有の目的やリスクに集中できます。一方で、評価内容まで一律に固定すると、案件に必要な観点や基準を柔軟に設定できなくなります。
