...
評価観点や5段階の判断基準を明確にし、評価者の感覚だけに依存しない状態を目指します。
...
この段階の目的は、すべての区分を必ず評価することではありません。評価対象になり得る領域を共通の地図として用意し、案件ごとの検討を同じ出発点から始められるようにすることです。

### 2. Done定義を決め、具体化の流れを共通化する

次に、区分2ごとに「評価を通して、どのような状態を目指すのか」をDone定義として整理しました。

例えば「誤情報・誤誘導」であれば、入力や参照情報と整合し、虚偽・誤りや利用者の誤認につながる内容を含まない状態をDoneとします。

評価区分の階層とDone定義までは、案件横断で利用する共通カタログとして用意しました。そのうえで、各案件では次の順番で評価内容を具体化します。
...
この構造により、共通の考え方を再利用しながら、案件の目的やリスクに必要な内容だけを設計できるようにしています。
...
一方で、Done定義の対象判断、評価観点、評価基準、Inputパターンなどをすべて固定してしまうと、案件固有の目的やリスクに対応できなくなります。
